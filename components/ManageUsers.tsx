          ...
          {usersInRoom?.docs.map((doc) => (
            <div key={doc.data().userId} className='flex justify-between items-center'>
              <p className='font-light'>
                {doc.data().userId === user?.emailAddresses[0].toString() ? `Você (${doc.data().userId})` : doc.data().userId}
              </p>
              <div className='flex items-center gap-2'>
            
                <Button variant='outline'>{doc.data().role}</Button>
                {isOwner && doc.data().userId !== user?.emailAddresses[0].toString() && (
                  <Button variant='outline' className='text-red-700 border-l-sidebar-accent-foreground border-r-sidebar-accent-foreground' disabled={isPending} size="sm" onClick={() => handleDelete(doc.data().userId)}>
